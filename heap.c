...
void _upheap(heap_t* heap, size_t pos_padre, size_t pos_swap, size_t pos_otro_hijo, cmp_func_t cmp){
	...
	heap->datos[pos_swap] = dato_ref;
	pos_ref = pos_padre;
	pos_padre = pos_swap;
	pos_swap = pos_ref;
	if(!prop_heap(heap, pos_otro_hijo)) _upheap(heap, pos_padre, pos_otro_hijo, pos_swap, cmp);
	size_t padre_del_padre = buscar_pos_padre(pos_padre);
	//Chequear lo de pasar pos_swap por parámetro acá
	_upheap(heap, padre_del_padre, pos_padre, pos_swap, cmp);
}
...
size_t calculo_maximo(heap_t* heap, cmp_func_t cmp, size_t pos_padre, size_t hijo_izq, size_t hijo_der){
	...
	if(heap->comparar(heap->datos[hijo_izq], heap->datos[hijo_der]) > 0){
		//hijo izq mayor que padre
		if(heap->comparar(heap->datos[hijo_izq], heap->datos[pos_padre]) > 0){
			return hijo_izq;
		}else{
			return pos_padre;
		}
	}else{
		if(heap->comparar(heap->datos[hijo_der], heap->datos[pos_padre]) > 0){
			return hijo_der;
		}else{
			return pos_padre;
		}
...
//Función recursiva que realiza el swap
void _downheap(heap_t* heap, size_t tam, size_t pos_padre, cmp_func_t cmp){
...
void downheap(heap_t* heap, cmp_func_t cmp){
	...
	_downheap(heap, heap->cant, 0, cmp);
}
